...
var FPC_ENC_MODULO=32768;var FPC_ENC_MODULO_RECIPROCAL=1/FPC_ENC_MODULO;var FPC_ENC_MAXLEN=function(){var l=Math.log(Math.pow(2,53))/Math.log(FPC_ENC_MODULO);// number of chars = words required to store worst case fp values
l+=1;l|=0;
//
...
return l}();var FPC_ENC_LOG2_TO_LOG10=1/Math.log2(10);
// mask: 0x000F; offset: 0xFFF0:
var FPC_ENC_POSITIVE_ZERO=65520;var FPC_ENC_NEGATIVE_ZERO=65521;var FPC_ENC_POSITIVE_INFINITY=65522;var FPC_ENC_NEGATIVE_INFINITY=65523;var FPC_ENC_NAN=65524;var FPC_DEC_POSITIVE_ZERO=0;var FPC_DEC_NEGATIVE_ZERO=-0;var FPC_DEC_POSITIVE_INFINITY=1/0;var FPC_DEC_NEGATIVE_INFINITY=-(1/0);var FPC_DEC_NAN=NaN;/*
...
//
if(flt){if(isFinite(flt)){
// encode sign in bit 12
var s;if(0>flt){s=4096;flt=-flt}else s=0;
// extract power from fp value    (WARNING: MSIE does not support log2(), see MDN!)
var exp2=Math.log2(flt);var p=0|exp2;// --> +1023..-1024, pardon!, +1024..-1074 (!!!)
if(-1024>p)
...
var y=flt/Math.pow(2,p);y/=4;// we do this in two steps to allow handling even the largest floating point values, which have p>=1023: Math.pow(2, p + 1) would fail for those!
if(y>=1)throw new Error("fp float encoding: mantissa above allowed max for "+flt);
// See performance test [test0008-array-join-vs-string-add]: string
// concatenation is the fastest cross-platform.
var a="";var b=y;if(0>b)throw new Error("fp encoding: negative mantissa for "+flt);if(0===b)throw new Error("fp encoding: ZERO mantissa for "+flt);
...
for(var i=0;b&&FPC_ENC_MAXLEN>i;i++){b*=FPC_ENC_MODULO;var c=0|b;// grab the integer part
var d=b-c;
//dbg[i + 1] = c;
//console.log('dbg @ step', i, c, flt, dbg, s, p, y, b, d, '0x' + c.toString(16));
a+=String.fromCharCode(c);b=d}
...
//
--i;if(i>3)throw new Error("fp encode length too large");var h=63488+p-1020+(s>>5)+(i<<5);// brackets needed as + comes before <<   :-(
if(63488>h||h>=63744)throw new Error("fp decimal long float near-inifinity number encoding: internal error: initial word out of range");a=String.fromCharCode(h)+a;
//dbg[0] = h;
...
var dp=exp2*FPC_ENC_LOG2_TO_LOG10+1|0;var dy=flt/Math.pow(10,dp-3);// take mantissa (which is guaranteed to be in range [0.999 .. 0]) and multiply by 1000
//console.log('decimal float test:', flt, exp2, exp2 * FPC_ENC_LOG2_TO_LOG10, p, dp, dy);
if(0>dy)throw new Error("fp decimal short float encoding: negative mantissa");if(0===dy)throw new Error("fp decimal short float encoding: ZERO mantissa");if(dy>1e3)throw new Error("fp decimal short float encoding: 3 digits check");
...
dc=32768+(dp<<11)+(s?1024:0)+(0|dy);// the `| 0` shouldn't be necessary but is there as a precaution
if(dc>=63488)throw new Error("fp decimal short float encoding: internal error: beyond 0xF800");if(dc>=55296&&57344>dc)throw new Error("fp decimal short float encoding: internal error: landed in 0xD8xx block");
...
var y=flt/Math.pow(2,p);if(y>=1)throw new Error("fp float encoding: mantissa above allowed max for "+flt);
// See performance test [test0008-array-join-vs-string-add]: string
// concatenation is the fastest cross-platform.
var a="";var b=y;// alt: y - 1, but that only gives numbers 0 < b < 1 for p > 0
if(0>b)throw new Error("fp encoding: negative mantissa for "+flt);if(0===b)throw new Error("fp encoding: ZERO mantissa for "+flt);
// and show the Unicode character codes for debugging/diagnostics:
//var dbg = [0 /* Note: this slot will be *correctly* filled at the end */];
//console.log('dbg @ start', 0, p + 1024 + s, flt, dbg, s, p, y, b);
for(var i=0;b&&FPC_ENC_MAXLEN>i;i++){b*=FPC_ENC_MODULO;var c=0|b;// grab the integer part
...
// - +0    (positive zero)
//
--i;if(i>3)throw new Error("fp encode length too large");var h=p+1024+s+(i<<13);// brackets needed as + comes before <<   :-(
if(h>=55296)throw new Error("fp decimal long float encoding: internal error: initial word beyond 0xD800");a=String.fromCharCode(h)+a;
//dbg[0] = h;
//console.log('dbg @ end', i, h, flt, dbg, s, p, y, b, '0x' + h.toString(16));
return a}
// -Inf / +Inf
// -Inf / +Inf
return flt>0?String.fromCharCode(FPC_ENC_POSITIVE_INFINITY):String.fromCharCode(FPC_ENC_NEGATIVE_INFINITY)}
// +0, -0 or NaN:
// +0, -0 or NaN:
// +0 --> 0, -0 --> PI
return isNaN(flt)?String.fromCharCode(FPC_ENC_NAN):Math.atan2(0,flt)?String.fromCharCode(FPC_ENC_NEGATIVE_ZERO):String.fromCharCode(FPC_ENC_POSITIVE_ZERO)}/*
...
//
opt=opt||{consumed_length:0};opt.consumed_length=1;var c0=s.charCodeAt(0);
...
case 55296:throw new Error("illegal fp encoding value in 0xD800-0xDFFF unicode range");case 63488:
// specials:
if(63744>c0){
...
// - 8..15: 0xF8
//
var len=96&c0;var vs=128&c0;var p=31&c0;p+=1020;
...
switch(len){case 0:
// 1 more 15-bit word:
im=s.charCodeAt(1);m=im/FPC_ENC_MODULO;opt.consumed_length++;
//console.log('decode-normal-len=1', m, s.charCodeAt(1));
break;case 32:
// 2 more 15-bit words:
im=s.charCodeAt(1);im<<=15;im|=s.charCodeAt(2);m=im/(FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=2;
...
im=s.charCodeAt(1);m=im/FPC_ENC_MODULO;im=s.charCodeAt(2);im<<=15;im|=s.charCodeAt(3);m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=3;
...
// so we'll have to use floating point for at least one intermediate step!
im=s.charCodeAt(1);im<<=15;im|=s.charCodeAt(2);m=im/(FPC_ENC_MODULO*FPC_ENC_MODULO);im=s.charCodeAt(3);im<<=15;im|=s.charCodeAt(4);
...
m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=4}
...
m*=4;m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
return m}switch(c0){case FPC_ENC_POSITIVE_ZERO:return 0;case FPC_ENC_NEGATIVE_ZERO:return-0;case FPC_ENC_POSITIVE_INFINITY:return 1/0;case FPC_ENC_NEGATIVE_INFINITY:return-(1/0);case FPC_ENC_NAN:return NaN;default:throw new Error("illegal fp encoding value in 0xF900-0xFFFF Unicode range")}break;case 32768:case 34816:case 36864:case 38912:case 40960:
...
//
var dm=1023&c0;// 10 bits
var ds=1024&c0;// bit 10 = sign
var dp=30720&c0;// bits 11..14: exponent
...
var sflt=dm/Math.pow(10,-dp);ds&&(sflt=-sflt);
//console.log('decode-short-1', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
return sflt;case 43008:case 45056:case 47104:case 49152:case 51200:case 53248:
...
var dp=30720&c0;// bits 11..14: exponent
//console.log('decode-short-0', ds, dm, '0x' + dp.toString(16), dp >>> 11, c0, '0x' + c0.toString(16));
dp>>>=11;dp-=5;var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
return sflt;
// (0xF900..0xFFF0: reserved for future use)
case 57344:case 59392:case 61440:
...
dp>>>=11;if(dp>=15)throw new Error("illegal fp encoding value in 0xF8xx-0xFFxx unicode range");dp-=6;// like above, but now also compensate for exponent bumping (0xB --> 0xC, ...)
var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
...
// - 15: 0 (zero)
//
var len=24576&c0;var vs=4096&c0;var p=4095&c0;p-=1024;
//console.log('decode-normal-0', vs, p, len, '0x' + len.toString(16), c0, '0x' + c0.toString(16));
// we don't need to loop to decode the mantissa: we know how much stuff will be waiting for us still
// so this is fundamentally an unrolled loop coded as a switch/case:
var m;var im;
...
m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=4}
//console.log('decode-normal-1', vs, m, p, opt.consumed_length);
m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
return m}}function encode_fp_value2(flt){
// sample JS code to encode a IEEE754 floating point value in a Unicode string.
//
// With provision to detect and store +0/-0 and +/-Inf and NaN
//
...
//       in order to prevent a collision with those Unicode specials at 0xF900..0xFFFF.
//
--i;var h=63488+p-1020+(s>>5)+(i<<5);// brackets needed as + comes before <<   :-(
...
case 0:if(!flt){
// +0, -0 or NaN:
if(isNaN(flt))return String.fromCharCode(FPC_ENC_NAN);// +0 --> 0, -0 --> PI
// +0 --> 0, -0 --> PI
return Math.atan2(0,flt)?String.fromCharCode(FPC_ENC_NEGATIVE_ZERO):String.fromCharCode(FPC_ENC_POSITIVE_ZERO)}if(!isFinite(flt))
// -Inf / +Inf
// -Inf / +Inf
return flt>0?String.fromCharCode(FPC_ENC_POSITIVE_INFINITY):String.fromCharCode(FPC_ENC_NEGATIVE_INFINITY);
// fall through!
...
case-1074:case-1073:case-1072:case-1071:case-1070:case-1069:case-1068:case-1067:case-1066:case-1065:case-1064:case-1063:case-1062:case-1061:case-1060:case-1059:case-1058:case-1057:case-1056:case-1055:case-1054:case-1053:case-1052:case-1051:case-1050:case-1049:case-1048:case-1047:case-1046:case-1045:case-1044:case-1043:case-1042:case-1041:case-1040:case-1039:case-1038:case-1037:case-1036:case-1035:case-1034:case-1033:case-1032:case-1031:case-1030:case-1029:case-1028:case-1027:case-1026:case-1025:-1024>p&&(
// Correct for our process: we actually want the bits in the IEE754 exponent, hence
// exponents lower than -1024, a.k.a. *denormalized zeroes*, are treated exactly
// like that in our code as well: we will produce leading mantissa ZERO words then.
p=-1024);
...
// - +0    (positive zero)
//
--i;var h=p+1024+s+(i<<13);// brackets needed as + comes before <<   :-(
a=String.fromCharCode(h)+a;
//dbg[0] = h;
//console.log('dbg @ end', i, h, flt, dbg, s, p, y, b, '0x' + h.toString(16));
return a}}function encode_fp_value3(flt){
// sample JS code to encode a IEEE754 floating point value in a Unicode string.
//
// With provision to detect and store +0/-0 and +/-Inf and NaN
//
...
// -Inf / +Inf
// -Inf / +Inf
return flt>0?String.fromCharCode(FPC_ENC_POSITIVE_INFINITY):String.fromCharCode(FPC_ENC_NEGATIVE_INFINITY)}else if(p>=1023){
// We also need to process the exponent +1024 specially as that is another edge case
// which we do not want to handle in our mainstream code flow where -1024 < p <= +1023
// maximum performance means we want the least number of conditional checks 
// (~ if/else constructs) in our execution path but I couldn't do without this extra one!
...
//dbg[0] = h;
//console.log('dbg @ end', i, h, flt, dbg, s, p, y, b, '0x' + h.toString(16));
return a}function decode_fp_value2(s,opt){
// sample JS code to decode a IEEE754 floating point value from a Unicode string.
//
// With provision to detect +0/-0 and +/-Inf and NaN
//
opt=opt||{consumed_length:0};var idx=opt.consumed_length;opt.consumed_length++;var c0=s.charCodeAt(idx);
//console.log('decode task: ', s, s.length, c0, '0x' + c0.toString(16));
// As we expect most encodings to be regular numbers, those will be in 0x0000..0x7FFF and
// we don't want to spend the least amount of time in the 'special values' overhead,
// which would be added overhead if we did check for those *first* instead of *at the same time*
...
switch(len){case 0:
// 1 more 15-bit word:
idx++;im=s.charCodeAt(idx);m=im/FPC_ENC_MODULO;opt.consumed_length++;
//console.log('decode-normal-len=1', m, s.charCodeAt(1));
break;case 32:
// 2 more 15-bit words:
idx++;im=s.charCodeAt(idx);im<<=15;idx++;im|=s.charCodeAt(idx);m=im/(FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=2;
//console.log('decode-normal-len=2', m, s.charCodeAt(1), s.charCodeAt(2));
break;case 64:
// 3 more 15-bit words: WARNING: this doesn't fit in an *integer* of 31 bits any more,
// so we'll have to use floating point for at least one intermediate step!
//
// Oh, by the way, did you notice we use a Big Endian type encoding mechanism?  :-)
idx++;im=s.charCodeAt(idx);m=im/FPC_ENC_MODULO;idx++;im=s.charCodeAt(idx);im<<=15;idx++;im|=s.charCodeAt(idx);m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);opt.consumed_length+=3;
//console.log('decode-normal-len=3', m, s.charCodeAt(1), s.charCodeAt(2), s.charCodeAt(3));
break;case 96:
// 4 more 15-bit words, where the last one doesn't use all bits. We don't use
// those surplus bits yet, so we're good to go when taking the entire word
...
// WARNING: this doesn't fit in an *integer* of 31 bits any more,
// so we'll have to use floating point for at least one intermediate step!
idx++;im=s.charCodeAt(idx);im<<=15;idx++;im|=s.charCodeAt(idx);m=im/(FPC_ENC_MODULO*FPC_ENC_MODULO);idx++;im=s.charCodeAt(idx);im<<=15;idx++;im|=s.charCodeAt(idx);
// Nasty Thought(tm): as we don't mask the lowest bits of that byte we MAY
// receive some cruft below the lowest significant bit of the encoded mantissa
// when we re-use those bits for other purposes one day. However, we can argue
// that we don't need to mask them bits anyway as they would disappear as
...
//
// With provision to detect +0/-0 and +/-Inf and NaN
opt?opt.consumed_length=1:opt={consumed_length:1};var c0=s.charCodeAt(0);
//console.log('decode task: ', s, s.length, c0, '0x' + c0.toString(16));
// As we expect most encodings to be regular numbers, those will be in 0x0000..0x7FFF and
// we don't want to spend the least amount of time in the 'special values' overhead,
// which would be added overhead if we did check for those *first* instead of *at the same time*
...
dp>>>=11;dp-=5;var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
return sflt}if(c0>=57344&&63488>c0){
...
var dp=30720&c0;// bits 11..14: exponent
//console.log('decode-short-0C', ds, dm, '0x' + dp.toString(16), dp >>> 11, c0, '0x' + c0.toString(16));
dp>>>=11;if(dp>=15)throw new Error("illegal fp encoding value in 0xF800-0xFFFF unicode range");dp-=6;// like above, but now also compensate for exponent bumping (0xB --> 0xC, ...)
var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1C', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
return sflt}if(63488>c0)
// This range spans the Unicode extended character ranges ('Surrogates') and MUST NOT be used by us for 'binary encoding'
// purposes as we would than clash with any potential Unicode validators out there! The key of the current
// design is that the encoded output is, itself, *legal* Unicode -- though admittedly I don't bother with
// the Unicode conditions surrounding shift characters such as these:
...
m*=4;m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
return m}switch(c0){case FPC_ENC_POSITIVE_ZERO:return 0;case FPC_ENC_NEGATIVE_ZERO:return-0;case FPC_ENC_POSITIVE_INFINITY:return 1/0;case FPC_ENC_NEGATIVE_INFINITY:return-(1/0);case FPC_ENC_NAN:return NaN;default:throw new Error("illegal fp encoding value in 0xF900-0xFFFF Unicode range")}}
...
// to fit snugly in the Unicode word range 0x8000..0xC000 or in a larger
// *decimal float* which spans two words: 13+15 bits.
var dp=exp2*FPC_ENC_LOG2_TO_LOG10+1|0;var dy=flt/Math.pow(10,dp-3);
//console.log('decimal float eligible? A:', flt, dy, chk, chk === dy, dp);
if((0|dy)===dy){
// alt check:   `(dy % 1) === 0`
// this input value is potentially eligible for 'short decimal float encoding'...
...
// +0, -0 or NaN:
// +0 --> 0, -0 --> PI
return isNaN(flt)?String.fromCharCode(FPC_ENC_NAN):Math.atan2(0,flt)?String.fromCharCode(FPC_ENC_NEGATIVE_ZERO):String.fromCharCode(FPC_ENC_POSITIVE_ZERO)}function decode_fp_value0(s,opt){
// sample JS code to decode a IEEE754 floating point value from a Unicode string.
//
// With provision to detect +0/-0 and +/-Inf and NaN
//
...
var dp=30720&c0;// bits 11..14: exponent
//console.log('decode-short-0C', ds, dm, '0x' + dp.toString(16), dp >>> 11, c0, '0x' + c0.toString(16));
dp>>>=11;dp-=6;// like above, but now also compensate for exponent bumping (0xB --> 0xC, ...)
var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1C', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
return sflt;default:
// 'regular' floating point values:
...
//
// With provision to detect +0/-0 and +/-Inf and NaN
var consumed_length;var c0=s.charCodeAt(0);
//console.log('decode task: ', s, s.length, c0, '0x' + c0.toString(16));
// As we expect most encodings to be regular numbers, those will be in 0x0000..0x7FFF and
// we don't want to spend the least amount of time in the 'special values' overhead,
// which would be added overhead if we did check for those *first* instead of *at the same time*
...
switch(len){case 0:
// 1 more 15-bit word:
im=s.charCodeAt(1);m=im/FPC_ENC_MODULO;consumed_length=2;
//console.log('decode-normal-len=1', m, s.charCodeAt(1));
break;case 8192:
// 2 more 15-bit words:
im=s.charCodeAt(1);im<<=15;im|=s.charCodeAt(2);m=im/(FPC_ENC_MODULO*FPC_ENC_MODULO);consumed_length=3;
//console.log('decode-normal-len=2', m, s.charCodeAt(1), s.charCodeAt(2));
break;case 16384:
// 3 more 15-bit words: WARNING: this doesn't fit in an *integer* of 31 bits any more,
// so we'll have to use floating point for at least one intermediate step!
//
// Oh, by the way, did you notice we use a Big Endian type encoding mechanism?  :-)
im=s.charCodeAt(1);m=im/FPC_ENC_MODULO;im=s.charCodeAt(2);im<<=15;im|=s.charCodeAt(3);m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);consumed_length=4;
//console.log('decode-normal-len=3', m, s.charCodeAt(1), s.charCodeAt(2), s.charCodeAt(3));
break;case 24576:
// 4 more 15-bit words, where the last one doesn't use all bits. We don't use
// those surplus bits yet, so we're good to go when taking the entire word
...
// that we don't need to mask them bits anyway as they would disappear as
// noise below the least significant mantissa bit anyway. :-)
m+=im/(FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO*FPC_ENC_MODULO);consumed_length=5}
//console.log('decode-normal-1', vs, m, p, opt.consumed_length);
m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
opt&&(opt.consumed_length=consumed_length);return m}
// We're entering the realm of 'short float' values, special codes and Unicode Surrogates...
if(43008>c0){
// 'short float' range 0x8000..0xA7FF
// 'human values' encoded as 'short floats' (negative decimal powers):
...
var sflt=dm/Math.pow(10,-dp);ds&&(sflt=-sflt);
//console.log('decode-short-1', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
opt&&(opt.consumed_length=1);return sflt}if(55296>c0){
// 'short float' range 0xA800..0xD7FF
// 'human values' encoded as 'short floats' (non-negative decimal powers):
//
// Bits in word:
...
dp>>>=11;dp-=5;var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
opt&&(opt.consumed_length=1);return sflt}if(c0>=57344&&63488>c0){
// 'short float' range 0xE000..0xF7FF
//
// (0xF900..0xFFF0: reserved for future use)
// 'human values' encoded as 'short floats':
...
var sflt=dm*Math.pow(10,dp);ds&&(sflt=-sflt);
//console.log('decode-short-1C', sflt, ds, dm, dp, c0, '0x' + c0.toString(16));
opt&&(opt.consumed_length=1);return sflt}if(63488>c0)
// This range spans the Unicode extended character ranges ('Surrogates') and MUST NOT be used by us for 'binary encoding'
// purposes as we would than clash with any potential Unicode validators out there! The key of the current
// design is that the encoded output is, itself, *legal* Unicode -- though admittedly I don't bother with
// the Unicode conditions surrounding shift characters such as these:
...
m*=4;m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
opt&&(opt.consumed_length=consumed_length);return m}opt&&(opt.consumed_length=1);switch(c0){case FPC_ENC_POSITIVE_ZERO:return 0;case FPC_ENC_NEGATIVE_ZERO:return-0;case FPC_ENC_POSITIVE_INFINITY:return 1/0;case FPC_ENC_NEGATIVE_INFINITY:return-(1/0);case FPC_ENC_NAN:return NaN;default:throw new Error("illegal fp encoding value in 0xF900-0xFFFF Unicode range")}}
//
...
var encode_fp_encode_lookup_table=encode_fp_init_encode_lookup_table();function encode_fp_value5(flt){
// sample JS code to encode a IEEE754 floating point value in a Unicode string.
//
// With provision to detect and store +0/-0 and +/-Inf and NaN
//
...
// +0, -0 or NaN:
// +0 --> 0, -0 --> PI
return isNaN(flt)?String.fromCharCode(FPC_ENC_NAN):Math.atan2(0,flt)?String.fromCharCode(FPC_ENC_NEGATIVE_ZERO):String.fromCharCode(FPC_ENC_POSITIVE_ZERO)}function decode_fp_value5(s,opt){
// sample JS code to decode a IEEE754 floating point value from a Unicode string.
//
// With provision to detect +0/-0 and +/-Inf and NaN
//
...
m*=Math.pow(2,p);vs&&(m=-m);
//console.log('decode-normal-2', m);
return m}}function encode_fp_init_lookup_tables(){// WARNING: off by +1 so that we can easily encode 'don't know' in here!
var first_pending_empty_slot=0;// WARNING: off by +1 so that we can easily encode 'don't know' in here!
var tbl=new Array(32768);for(var c0=63488;65536>c0;c0++)tbl[c0-32768]=!1;for(var c0=55296;57344>c0;c0++)tbl[c0-32768]=!1;tbl[FPC_ENC_POSITIVE_ZERO-32768]=0;tbl[FPC_ENC_NEGATIVE_ZERO-32768]=0;tbl[FPC_ENC_POSITIVE_INFINITY-32768]=1/0;tbl[FPC_ENC_NEGATIVE_INFINITY-32768]=1/0;tbl[FPC_ENC_NAN-32768]=NaN;for(var c0=32768;63488>c0;c0++)switch(63488&c0){
// This range spans the Unicode extended character ranges ('Surrogates') and MUST NOT be used by us for 'binary encoding'
// purposes as we would than clash with any potential Unicode validators out there! The key of the current
// design is that the encoded output is, itself, *legal* Unicode -- though admittedly I don't bother with
// the Unicode conditions surrounding shift characters such as these:
...
// 
// which reside in the other ranges that we DO employ for our own nefarious encoding purposes!
case 55296:first_pending_empty_slot||(first_pending_empty_slot=c0+1);c0=57342;// speed up the loop!
break;case 32768:case 34816:case 36864:case 38912:case 40960:
// 'human values' encoded as 'short floats' (negative decimal powers):
//
// Bits in word:
// - 0..9: integer mantissa; values 0..1023
...
if(100>dm){first_pending_empty_slot||(first_pending_empty_slot=c0+1);tbl[c0-32768]=!1;continue}if(dm>=1e3){first_pending_empty_slot||(first_pending_empty_slot=c0+1);tbl[c0-32768]=!1;continue}
//console.log('decode-short-0', ds, dm, '0x' + dp.toString(16), dp >>> 11, c0, '0x' + c0.toString(16));
dp>>>=11;dp-=5;
// Because `dm * Math.pow(10, dp)` causes bitrot in LSB (so that, for example, input value 0.0036 becomes 0..0036000000000000003)
// we reproduce the value another way, which does *not* produce the bitrot in the LSBit of the *decimal* mantissa:
...
//  
function __find_fp_radix_hash_settings__(){var M,B,a,collisions,minv,maxv;var B=Math.log2(.1*Math.pow(10,-2));for(var M=1;1500>=M;M+=1){a=[];collisions=0;minv=1/0;maxv=-(1/0);for(var i=-3;12>i;i++)for(var j=100;1e3>j;j++){var v=j/1e3*Math.pow(10,i+1);minv=Math.min(v,minv);maxv=Math.max(v,maxv);
// the hash math under test:
var y=(Math.log2(v)-B)*M;var z=0|y;if(a[z]){collisions++;if(collisions>20)break}else a[z]=v}if(!collisions)break}return M}var FPCVT_LUT_M=__find_fp_radix_hash_settings__();function encode_fp_init_decode_lookup_table(){}
// mapping floating point value to Unicode character code () to fp value or FALSE
...
var test_serialization=!0;var data=[];var serialized_data=[];var serialized_data2=[];var serialized_data3=[];var data_length=0;
// console.clear();
function init(){if(!data_length){
//debugger;
data.push(1e-5);data.push(3676483910);data.push(1.9999999999999998*Math.pow(2,1023));data.push(1.7976931348623157e308);// == 1.7976931348623157e308
data.push(1.7976931348623157e308);data.push(1.7976931348623155e308);// == 1.7976931348623155e308
data.push(.999999999999999*Math.pow(2,-1024));data.push(.999999999999999*Math.pow(2,-1025));data.push(.999999999999999*Math.pow(2,-1023));data.push(1.999999999999998*Math.pow(2,1023));data.push(1.999999999999999*Math.pow(2,1022));data.push(1.999999999999998*Math.pow(2,1022));data.push(.999999999999999*Math.pow(2,1023));data.push(.999999999999999*Math.pow(2,1022));data.push(.999999999999999*Math.pow(2,1021));data.push(.999999999999999*Math.pow(2,1020));data.push(.999999999999999*Math.pow(2,1019));data.push(1.999999999999998*Math.pow(2,1019));data.push(2*Math.pow(2,1023));// +Infinity
data.push(1.9999999999999998*Math.pow(2,1023));data.push(1.9999999999999998*Math.pow(2,1023));data.push(5e-324);data.push(1.5e-323);data.push(2.5e-323);data.push(3.5e-323);data.push(4.2388366884150523e-308);data.push(28e9);data.push(.036);data.push(.36);data.push(3.6);data.push(.0036);data.push(36e-5);data.push(36e-6);data.push(1.5e-320);data.push(NaN);data.push(0);data.push(-0);data.push(+(1/0));data.push(-(1/0));for(var i=0,l=1e3;l>i;i++){var x=Math.random();var y=Math.tan(x-.5);var z=Math.pow(2,100*y);var a=1+21*Math.random()|0;var b=z.toPrecision(a);data.push(parseFloat(b))}
// test all powers of 2 which fit in a floating point value
for(var i=-1074;1023>=i;i++){var z=Math.pow(2,i);data.push(z)}for(var i=-1074;1023>=i;i++)for(var j=0;20>j;j++){var x=Math.random();var y=x/20+j/20+1;// stay within the next power of 2: 1.0000 .. 1.9999
var z=Math.pow(2,i)*y;data.push(z)}for(var i=-1074;-1022>=i;i++)for(var j=0;50>j;j++){var x=Math.random();var z=Math.pow(2,i)*x;data.push(z)}for(var i=1017;1023>=i;i++)for(var j=0;50>j;j++){var x=Math.random();var y=x+1;// stay within the next power of 2: 1.0000 .. 1.9999
var z=Math.pow(2,i)*y;data.push(z)}
...
for(var i=32768;63744>i;i++)if(55296>i||i>57343){var t=String.fromCharCode(i);var z=decode_fp_value(t);
// and test if the *encoder* handles all these 'short notation' samples correctly:
var s=encode_fp_value(z);s.length!==t.length;data.push(z)}for(var i=-6;16>=i;i++)for(var j=0;1e3>j;j++){var x=Math.random();var y=x/2+.5;// stay within the current power of 2
var z=Math.pow(2,i)*y;var w=6*Math.random()|0;var b;switch(w){case 0:b=z.toPrecision(3);break;case 1:b=z.toPrecision(2);break;case 2:b=z.toPrecision(1);break;case 3:b=z.toPrecision(4);break;case 4:b=z.toPrecision(5)}data.push(parseFloat(b))}data.length=30;data_length=data.length;"undefined"!=typeof console}}
// serialize / deserialize functions:
function classic_1(data,len){for(var i=0;len>i;i++){var flt=data[i];var s=""+flt;// fastest solution for encode
var t=parseFloat(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_0(data,len){
//debugger;
var dec_opt={consumed_length:0};for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value(flt);dec_opt.consumed_length=0;var t=decode_fp_value(s,dec_opt);/* test_serialization && */!(t===flt||isNaN(t)&&isNaN(flt));dec_opt.consumed_length!==s.length;var s2=encode_fp_value0(flt);s2=encode_fp_value2(flt);s2=encode_fp_value3(flt);s2=encode_fp_value4(flt);dec_opt.consumed_length=0;var t2=decode_fp_value2(s,dec_opt);!(t2===flt||isNaN(t2)&&isNaN(flt));dec_opt.consumed_length!==s.length;dec_opt.consumed_length=0;t2=decode_fp_value3(s,dec_opt);!(t2===flt||isNaN(t2)&&isNaN(flt));dec_opt.consumed_length!==s.length;dec_opt.consumed_length=0;t2=decode_fp_value4(s,dec_opt);!(t2===flt||isNaN(t2)&&isNaN(flt));dec_opt.consumed_length!==s.length}}function custom_1(data,len){
//debugger;
for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value0(flt);var t=decode_fp_value(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_2(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value2(flt);var t=decode_fp_value(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_3(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value3(flt);var t=decode_fp_value(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_4(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value4(flt);var t=decode_fp_value(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_5(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value(flt);var t=decode_fp_value2(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_6(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value2(flt);var t=decode_fp_value2(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_7(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value3(flt);var t=decode_fp_value2(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_8(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value4(flt);var t=decode_fp_value2(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_9(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value(flt);var t=decode_fp_value3(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_10(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value2(flt);var t=decode_fp_value3(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_11(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value3(flt);var t=decode_fp_value3(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_12(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value4(flt);var t=decode_fp_value3(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_13(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value(flt);var t=decode_fp_value4(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_14(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value2(flt);var t=decode_fp_value4(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_15(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value3(flt);var t=decode_fp_value4(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}function custom_16(data,len){for(var ii=0;len>ii;ii++){var flt=data[ii];var s=encode_fp_value4(flt);var t=decode_fp_value4(s);test_serialization&&t!==flt&&(!isNaN(t)||!isNaN(flt))}}init();
...
// Custom : v16'
custom_16(data,data_length,serialized_data);describe("Give it some context",function(){describe("maybe a bit more context here",function(){it("should run here few assertions",function(){expect(!0).toBe(!0)})})});